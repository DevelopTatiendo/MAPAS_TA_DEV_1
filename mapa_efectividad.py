...
def get_dynamic_gradient(efectividad_values):
    ...
    return gradient
def generar_mapa_efectividad(ciudad):
    ...
    if ciudad in rutas_coordenadas:
        ...
        df_efectividad = pd.read_excel(ruta_coordenadas) 
    ...
    function(cluster) {
        ...
        var totalEfectividad = 0;
        var count = 0;

        // Calcular el promedio de efectividad en el clúster
        markers.forEach(function(marker) {
            if (marker.options.efectividad) {
                totalEfectividad += marker.options.efectividad;
                count++;
            }
        });

        var promedio = count > 0 ? (totalEfectividad / count).toFixed(2) : 0;
    ...
    cantidad_barrios = df_efectividad['barrio'].nunique()
    promedio_efectividad = df_efectividad['efectividad'].mean()
   ...
   # Obtener el gradiente dinámico basado en la distribución
    gradient = get_dynamic_gradient(df_efectividad['efectividad'].values)
    
    # Normalizar los datos para el heatmap
    heat_data = df_efectividad[['latitud', 'longitud', 'efectividad']].values
    min_efectividad = heat_data[:, 2].min()
    max_efectividad = heat_data[:, 2].max()
    heat_data[:, 2] = (heat_data[:, 2] - min_efectividad) / (max_efectividad - min_efectividad)
    ...
    for _, row in df_efectividad.iterrows():
        popup_text = f"Barrio: {row['barrio']} {row['efectividad']:.2f}"
        ...
        marker.options['efectividad'] = row['efectividad']
        marker.add_to(marker_cluster)
    
    filename = f"mapa_efectividad_{ciudad}.html"
    filepath = f"static/maps_densidad/efectividad/{filename}"
    mapa.save(filepath)
    return filename  
...
for ciudad in lista:
    filename = generar_mapa_efectividad(ciudad)
